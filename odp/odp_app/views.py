...
def do_paging_response(request, obj, template):
    ...
    resp = render_to_response(
        template,
        {"obj": obj, "query": query, "pag": pag},
        context_instance=RequestContext(request),
    )
    if (not request.COOKIES.has_key("quante")) or (
...
def new_s_results(request):
    ...
    if not any(
        [request.GET.has_key(par) and request.GET[par] for par in par_danneggiato]
    ):
    ...
    try:
        ...
        if request.GET.has_key("est_it"):
            obj = obj.filter(est_it=True)
        if request.GET.has_key("est_dm"):
            obj = obj.filter(est_dm=True)
        if request.GET.has_key("danno_morte"):
            obj = obj.filter(dm_est=True)
        if request.GET.has_key("danno_np"):
        ...
        if (
            request.GET.has_key("grado_di_giudizio")
            and request.GET["grado_di_giudizio"]
        ):
            obj = obj.filter(
                sentenza__grado_di_giudizio__contains=request.GET["grado_di_giudizio"]
            )
        if (
            request.GET.has_key("data_della_sentenza")
            and request.GET["data_della_sentenza"]
        ):
            obj = obj.filter(
                sentenza__data_della_sentenza=request.GET["data_della_sentenza"]
            )
        if (
            request.GET.has_key("data_del_deposito")
            and request.GET["data_del_deposito"]
        ):
            obj = obj.filter(
                sentenza__data_del_deposito=request.GET["data_del_deposito"]
            )
        if (
            request.GET.has_key("numero_della_sentenza")
            and request.GET["numero_della_sentenza"]
        ):
            obj = obj.filter(
                sentenza__numero_della_sentenza=request.GET["numero_della_sentenza"]
            )
        if (
            request.GET.has_key("anno_del_deposito")
            and request.GET["anno_del_deposito"]
        ):
            obj = obj.filter(
        ...
        if request.GET.has_key("sede_tribunale") and request.GET["sede_tribunale"]:
            obj = obj.filter(
                sentenza__sede_tribunale__comune__icontains=request.GET[
                    "sede_tribunale"
                ]
            )
        ...
        if (
            request.GET.has_key("contenuto_testuale")
            and request.GET["contenuto_testuale"]
        ):
        ...
        return render_to_response(
            "errore.html", {"mess": mess}, context_instance=RequestContext(request)
        )
    else:
...
def i_results(request):
    ...
    try:
        if request.GET.has_key("eta") and request.GET["eta"]:
        ...
        if request.GET.has_key("sesso") and request.GET["sesso"]:
...
def d_results(request):
    ...
    try:
        ...
        if request.GET.has_key("sede_tabella"):
            obj = obj.filter(sede_tabella=request.GET["sede_tabella"])

        if request.GET.has_key("est_clg"):
            obj = obj.filter(est_clg=True)
        if request.GET.has_key("est_cls"):
            obj = obj.filter(est_cls=True)

        if request.GET.has_key("est_lcip"):
        ...
        if request.GET.has_key("est_lcit"):
...
def new_search(request):
    return render_to_response(
        "odp/tmp_search.html", {}, context_instance=RequestContext(request)
    )
...
s_results = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    s_results
)
new_s_results = user_passes_test(
    lambda u: u.is_authenticated(), login_url=var_login_url
)(new_s_results)
s_details = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    s_details
)
i_results = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    i_results
)
i_details = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    i_details
)
d_results = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    d_results
)
search_noscript = user_passes_test(
    lambda u: u.is_authenticated(), login_url=var_login_url
)(search_noscript)
search = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    search
)
new_search = user_passes_test(lambda u: u.is_authenticated(), login_url=var_login_url)(
    new_search
)
