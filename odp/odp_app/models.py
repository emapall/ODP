...
class FattoreLiquidazione(models.Model):
	...
	class Meta:
		...
		verbose_name_plural = u'fattori rilevanti di liquidazione del danno non patrimoniale'
		ordering = ("fattore",)
...
class Sentenza(models.Model):
	...
	grado_di_giudizio = models.CharField('grado di giudizio', max_length=1, default='', choices=GRADO_DI_GIUDIZIO, null=False, blank=False)
	numero_della_sezione = models.IntegerField('numero della sezione', null=False, blank=False, default=0)
	...
	numero_della_sentenza = models.IntegerField('numero della sentenza', null=False, blank=False)
	esaminatore = models.ManyToManyField(Esaminatore, verbose_name='esaminatore', null=True, blank=False)
	osservatorio = models.ManyToManyField(Osservatorio, verbose_name='osservatorio', null=True, blank=False)
	estensore = models.CharField('estensore', max_length=50, null=False, blank=False)
	riconvenzionale = models.CharField('riconvenzionale', max_length=1, choices=RICONVENZIONALE, null=False, blank=False)
...
#	profili_rilevanti = models.ManyToManyField(ProfiloRilevante, verbose_name='profili rilevanti', null=True, blank=True)
	note_profili_rilevanti = models.TextField('note ai profili rilevanti', null=True, blank=True)
	...
	sede_tribunale = models.ForeignKey(Comune, verbose_name='sede tribunale', null=True, blank=False)
	assicurazione = models.ManyToManyField(Assicurazione, verbose_name='assicurazione', null=True, blank=False) #filter_interface=models.HORIZONTAL,
	provenienza = models.ForeignKey(Provenienza, verbose_name='provenienza', null=True, blank=True)
	numero_attori = models.IntegerField('numero attori', default=0, null=True, blank=False)
	numero_convenuti = models.IntegerField('numero convenuti', default=0, null=True, blank=False)
	numero_terzi = models.IntegerField('numero terzi', default=0, null=True, blank=False)
	fatto = models.TextField('fatto', null=False, blank=False)
	responsabilita = models.ForeignKey(Responsabilita, verbose_name='responsabilità', null=True, blank=False)
	note_sentenza = models.TextField('note', null=False, blank=True)
	ocr = models.TextField('ocr', null=False, blank=True)
	file_img = models.FileField('immagine', upload_to='immagine/', null=True, blank=True)
	file_sch = models.FileField('scheda riassuntiva', upload_to='scheda/', null=True, blank=True)
	file_cmn = models.FileField('scheda di commento', upload_to='commento/', null=True, blank=True)
	
	# Flag se escludere la sentenza dalla vista del mondo
	forza_esclusione = models.BooleanField('Rendi la sentenza non pubblicabile, indipendentemente dalle schede infortunato', default=False)
...
#				linestr = '(' + link[1+link.rfind('/'):link.rfind('.')] + ') '
		...
		if self.grado_di_giudizio == u'':
			...
			linestr = self.get_grado_di_giudizio_display() + u' di ' + self.sede_tribunale.comune;
...
class Infortunato(models.Model):
	...
	sentenza =                 models.ForeignKey(Sentenza, verbose_name='sentenza', related_name='infortunati')
	eta =                      models.IntegerField('età', null=True, blank=True)
	est_maggiorenne =          models.CharField('est maggiorenne', max_length=1, choices=MAGGIORENNE, null=True, blank=False)
	sesso =                    models.CharField('sesso', max_length=1, choices=SESSO, null=True, blank=False)
	professione =              models.ForeignKey(Professione, verbose_name='professione')
	percentuale_colpa_attore = models.DecimalField('percentuale colpa attore',  decimal_places=2, max_digits=5, null=True, blank=False)
	ente_sociale =             models.CharField('ente sociale', max_length=25, choices=ENTE_SOCIALE, null=True, blank=True)
	tipo_sr =                  models.CharField('tipo surroga', max_length=1, choices=TIPO_SR, null=True, blank=True)
	importo_sr =               models.DecimalField('importo surroga', decimal_places=2, max_digits=20, null=True, blank=True)
	
	##### campi tra tonde da inserire qua #????
	
	importo_provvisionale = models.DecimalField('importo acconto', decimal_places=2, max_digits=20, null=True, blank=False)
	tipo_rivalutazione =    models.CharField('tipo rivalutazione', max_length=1, choices=TIPO_RIVALUTAZIONE, null=True, blank=False)

	interessi_legali_it =   models.DateField('interessi legali invalidità temporanea', null=True, blank=True)
	interessi_legali_ip =   models.DateField('interessi legali invalidità permanente', null=True, blank=True)
	interessi_legali_dm =   models.DateField('interessi legali danni morali', null=True, blank=True)
	interessi_legali_ss =   models.DateField('interessi legali spese sostenute', null=True, blank=True)
	interessi_legali_dc =   models.DateField('interessi legali danni alle cose', null=True, blank=True)
	interessi_legali_lcit = models.DateField('interessi legali lucro cessante invalidità temporanea', null=True, blank=True)
	interessi_legali_lcip = models.DateField('interessi legali lucro cessante invalidità permanente', null=True, blank=True)
	...
	est_das_it =                  models.CharField('danno biologico da invalidità temporanea', max_length=1, null=True, blank=True, editable=False)
	metodo_das_it =               models.CharField('metodo das it', max_length=1, choices=DASIT, null=True, blank=True)
	importo_liquidazione_das_it = models.DecimalField('importo liquidazione das it', decimal_places=2, max_digits=20, null=True, blank=True)
	euro_al_di =                  models.DecimalField('euro al dì', decimal_places=2, max_digits=20, null=True, blank=True)
	est_lcit =                    models.BooleanField('c\'è lucro cessante da invalidità temporanea', null=False, blank=True)
	metodo_lcit =                 models.CharField('metodo lucro cessante da invalidità temporanea', max_length=1, choices=LCIT, null=True, blank=True)
	importo_liquidazione_lcit =   models.DecimalField('importo lucro cessante da invalidità temporanea', decimal_places=2, max_digits=20, null=True, blank=True)
	giorni_lcit =                 models.IntegerField('giorni lucro cessante da invalidità temporanea', null=True, blank=True)
	
	est_ip =                      models.BooleanField('c\'è invalidità permanente')
	est_das_ip =                  models.CharField('danno biologico da invalidità permanente', max_length=1, null=True, blank=True)
	metodo_das_ip =               models.CharField('metodo das ip', max_length=1, choices=DASIP, null=True, blank=True)
	percentuale_das_ip =          models.DecimalField('percentuale das ip', decimal_places=2, max_digits=5, null=True, blank=True)
	importo_liquidazione_das_ip = models.DecimalField('importo liquidazione das ip', decimal_places=2, max_digits=20, null=True, blank=True)
	est_lcip =                    models.BooleanField('c\'è lucro cessante da invalidità permanente', null=False, blank=True)
	metodo_lcip =                 models.CharField('metodo lucro cessante da invalidità permanente', max_length=1, choices=LCIT, null=True, blank=True)
	percentuale_lcip =            models.DecimalField('percentuale', decimal_places=2, max_digits=5, null=True, blank=True)
	importo_liquidazione_lcip =   models.DecimalField('importo lucro cessante da invalidità permanente', decimal_places=2, max_digits=20, null=True, blank=True)
	...
	metodo_di_quantificazione = models.CharField('metodo di quantificazione', max_length=255, null=True, blank=True)
	periodo_di_sopravvivenza = models.CharField('periodo di sopravvivenza', max_length=255, null=True, blank=True)
	criterio_liquidazione  = models.CharField('criterio di liquidazione', max_length=255, null=True, blank=True)
	...
	est_interazione = models.BooleanField('est interazione tra risarcimento danno patrimoniale e risarcimento danno non patrimoniale')
	sunt_diritti_lesi = models.BooleanField('ci sono diritti lesi')

	percentuale_pddb =          models.DecimalField('% danno',  decimal_places=2, max_digits=5, null=True, blank=True)
	...
	importo_liquidazione_dm =   models.DecimalField('importo liquidazione danni morali', decimal_places=2, max_digits=20, null=True, blank=True)
	...
	importo_liquidazione_dmit = models.DecimalField('Importo danni morali da I.T.', decimal_places=2, max_digits=20, null=True, blank=True)
	durata_dmit =               models.IntegerField('Durata danni morali da I.T.', null=True, blank=True)
	importo_liquidazione_dmip = models.DecimalField('importo danni morali da I.P.', decimal_places=2, max_digits=20, null=True, blank=True)
	percentuale_dmip =          models.DecimalField('% danni morali da I.P.',  decimal_places=2, max_digits=5, null=True, blank=True)
	est_ss =                    models.BooleanField('risarcite spese sostenute')
	est_ss_sostenute =          models.BooleanField('risarcite spese già sostenute')
	importo_sostenute =         models.DecimalField('importo spese già sostenute', decimal_places=2, max_digits=20, null=True, blank=True)
	est_ss_future =             models.BooleanField('risarcite spese future')
	importo_future =            models.DecimalField('importo spese future', decimal_places=2, max_digits=20, null=True, blank=True)
	importo_danni_cose =        models.DecimalField('importo danni alle cose', decimal_places=2, max_digits=20, null=True, blank=True)
	importo_altri_danni =       models.DecimalField('importo altri danni', decimal_places=2, max_digits=20, null=True, blank=True)
	altri_danni =               models.TextField('altri danni', null=True, blank=True)
	tipo_lesione =              models.CharField('tipo lesione', max_length=1, choices=TIPO_LESIONE, null=True, blank=False)
	accoglimento_giudice =      models.CharField('accoglimento giudice', max_length=1, choices=ACCOGLIMENTO, null=True, blank=True)
	# mettere una lista?
	qualifica_ctu = 			models.CharField('qualifica ctu', max_length=25, null=True, blank=True)
	tabella = 					models.BooleanField('tabella')
	sede_tabella = 				models.CharField('sede tabella', max_length=25, null=True, blank=True)
	note_fattori_rilevanti_liquidazione = models.TextField('note ai fattori rilevanti di liquidazione del danno non patrimoniale', null=True, blank=True)
	note_fattori_rilevanti_liquidazione_dp = models.TextField('note ai fattori rilevanti di liquidazione del danno patrimoniale', null=True, blank=True)
	
	richiestaparteattrice =     models.ManyToManyField(RichiestaParteAttrice, null=False, blank=False, verbose_name='Richiesta parte attrice')
	
	dm_est =                       models.BooleanField('est danno da morte')
	dm_dbjh_est =                  models.BooleanField('est danno biologico jure hereditario')
	dm_dbjh_importo_liquidazione = models.DecimalField('importo liquidazione danno biologico jure hereditario', decimal_places=2, max_digits=20, null=True, blank=True)
	...
	tipo_ctu=models.CharField('tipo ctu', max_length=2, choices=TIPO_CTU, null=True, blank=True)
	tipo_quesito=models.CharField('tipo quesito', max_length=2, choices=TIPO_QUESITO, null=True, blank=True)
	resoconto_op_per=models.CharField('resoconto op peritali', max_length=2, choices=RESOCONTO_OP_PERITALI, null=True, blank=True)
	tipo_ctp=models.CharField('tipo ctp', max_length=2, choices=TIPO_CTP, null=True, blank=True)
	
	ctp_nominati=models.CharField('ctp nominati', max_length=2, choices=CTP_NOMINATI, null=True, blank=True)
	tipo_doc_medica=models.CharField('tipo documentazione medica', max_length=2, choices=TIPO_DOC_MEDICA, null=True, blank=True)
	tipo_ana_generale=models.CharField('tipo anamnesi generale', max_length=2, choices=TIPO_ANA_GENERALE, null=True, blank=True)
	tipo_ana_lavorativa=models.CharField('tipo anamnesi lavorativa', max_length=2, choices=TIPO_ANA_LAVORATIVA, null=True, blank=True)
	tipo_stato_attuale=models.CharField('tipo stato attuale', max_length=2, choices=TIPO_STATO_ATTUALE, null=True, blank=True)
	tipo_eo_generale=models.CharField('tipo esame obiettivo generale', max_length=2, choices=TIPO_EO_GENERALE, null=True, blank=True)
	tipo_eo_specifico=models.CharField('tipo esame obiettivo specifico', max_length=2, choices=TIPO_EO_SPECIFICO, null=True, blank=True)
	#TODO:checkbox (sì/ no)
	flag_vis_spec=models.CharField('richiesta visita specialistica', max_length=2, choices=FLAG_VIS_SPEC, null=True, blank=True)
	tipo_esami_strumentali=models.CharField('tipo esami strumentali', max_length=2, choices=TIPO_ESAMI_STRUMENTALI, null=True, blank=True)
	tipo_trat_ml=models.CharField('tipo trattazione medico legale', max_length=2, choices=TIPO_TRAT_ML, null=True, blank=True)
	tipo_risposta_quesiti=models.CharField('tipo risposta quesiti', max_length=2, choices=TIPO_RISPOSTA_QUESITI, null=True, blank=True)
	...
	desc_rif_tabellare=models.TextField('descrizione riferimento tabellare', null=True, blank=True)
	...
	### campi calcolati ###
	importo_liquidazione_das = models.DecimalField('liquidazione danno alla salute (dasip+dasit)', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	importo_dap = models.DecimalField('totale danni alla persona', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	importo_ad = models.DecimalField('totale altri danni', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	importo_totale = models.DecimalField('totale', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	importo_liquidato = models.DecimalField('totale liquidato', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	liquidazione_totale = models.DecimalField('liquidazione totale', decimal_places=2, max_digits=20, null=True, blank=True, editable=False)
	# FIXME: da calcolare?
	importo_rivalutazione = models.DecimalField('importo rivalutazione', decimal_places=2, max_digits=20, null=True, blank=True)
	
	# campi aggiunti per alleggerire le query [JC 17/3/2007]
	pre2001=models.BooleanField('Risale a prima del 2001', db_index=True, default=False)
	
	# molti a molti
	lesione=models.ManyToManyField(Lesione, null=True, blank=True) #filter_interface=models.VERTICAL,
	postumo=models.ManyToManyField(Postumo, null=True, blank=True) #filter_interface=models.VERTICAL,	
	postumo_tabulato=models.ManyToManyField(Postumo_tabulato, null=True, blank=True)
	
	# Altri danni patrimoniali
	dannopatrimoniale = models.ManyToManyField(DannoPatrimoniale, null=True, blank=True, verbose_name='Altri danni patrimoniali')
	importo_dannopatrimoniale = models.DecimalField('importo liquidato per altri danni patrimoniali', decimal_places=2, max_digits=20, null=True, blank=True)
	interessi_dannopatrimoniale = models.DateField('interessi legali per altri danni patrimoniali', null=True, blank=True)
	note_dannopatrimoniale=models.TextField('note per altri danni patrimoniali', null=True, blank=True)
	note_exaltrespese=models.TextField('note per spese e danni alle cose', null=True, blank=True)
	...
	dirittoinviolabile = models.ManyToManyField(DirittoInviolabile, null=True, blank=True, verbose_name='Diritti lesi')
	importo_dirittoinviolabile = models.DecimalField('importo liquidato', decimal_places=2, max_digits=20, null=True, blank=True)
	interessi_dirittoinviolabile = models.DateField('interessi legali', null=True, blank=True)
	...
	fattori_rilevanti_liquidazione = models.ManyToManyField(FattoreLiquidazione, null=True, blank=True, verbose_name='fattori rilevanti di liquidazione del danno non patrimoniale')
	fattori_rilevanti_liquidazione_dp = models.ManyToManyField(FattoreLiquidazioneDP, null=True, blank=True, verbose_name='fattori rilevanti di liquidazione del danno patrimoniale')
	prova_del_dnp = models.ManyToManyField(ProvaDelDNP, null=True, blank=True, verbose_name='prove del danno non patrimoniale')
	note_prova_del_dnp = models.TextField('note alle prove del danno non patrimoniale', null=True, blank=True)
	prova_del_dp = models.ManyToManyField(ProvaDelDP, null=True, blank=True, verbose_name='prove del danno patrimoniale')
	note_prova_del_dp = models.TextField('note alle prove del danno patrimoniale', null=True, blank=True)
	trend_liquidazione = models.ManyToManyField(TrendLiquidazione, null=True, blank=True, verbose_name='trend di liquidazione')
	note_trend_liquidazione = models.TextField('note ai trend di liquidazione', null=True, blank=True)
...
#################################################################################################################
		...
		termini = [self.importo_liquidazione_das, self.dm_dbjh_importo_liquidazione, \
			self.importo_liquidazione_lcit, self.importo_liquidazione_lcip, \
			self.importo_liquidazione_dmit, self.importo_liquidazione_dmip, \
			self.importo_liquidazione_dm, self.importo_dirittoinviolabile ]
		self.importo_dap = sum( termine for termine in termini if termine )
		
		# in 1557, importo_ad è zero e non dovrebbe
		termini = [self.importo_danni_cose, self.importo_altri_danni, self.importo_dannopatrimoniale]
		...
		termini = [self.importo_dap, \
			self.importo_sostenute, self.importo_future, self.importo_ad]
...
class TrendProfiloRilevanteContainer(models.Model):
	trend = models.ForeignKey(TrendProfiloRilevante, db_index = True, verbose_name='parola chiave')
	profili_rilevanti = models.ManyToManyField(ProfiloRilevante, db_index = True, blank = True, null = True, verbose_name='profili rilevanti')
